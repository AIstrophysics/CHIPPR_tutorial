...
def plot_true_histogram(true_samps, n_bins, plot_loc, prepend, plot_name):
    """
    Plot the true histogram of sample values.

    Parameters
    ----------
    true_samps : array-like
        Array of true sample values.
    n_bins : tuple
        Number of bins for the histogram (coarse and fine).
    plot_loc : str
    ...
    # Replace `normed` with `density`
    sps.hist(true_samps, bins=n_bins[1], density=True, color='k', alpha=0.5, log=True)
    sps.hist(true_samps, bins=n_bins[0], density=True, color='y', alpha=0.5, log=True)
    
    #sps.set_xlabel(r'$z_{true}$')
    #sps.set_ylabel(r'$n(z_{true})$')

    sps.set_xlabel(r'$z_{true}$')
    sps.set_ylabel('Density')
    plt.savefig(f"{plot_loc}/{prepend}_{plot_name}.png")
    plt.close(f)
