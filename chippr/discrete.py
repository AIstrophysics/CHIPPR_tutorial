...
from pomegranate.distributions import Uniform as UD
...
class discrete(object):
    def __init__(self, bin_ends, weights):
        ...
        # Ensure compatibility with pomegranate by passing `mins` and `maxs` as 1D arrays
        self.funcs = [
            UD(np.array([self.bin_ends[i]]), np.array([self.bin_ends[i + 1]]))
            for i in self.bin_range
        ]
